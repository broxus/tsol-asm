...
const ROUNDING_MODE_SUFFIXES: [&str; 3] = [
    "",  // Floor
    "R", // Nearest integer
    "C", // Ceiling
];
...
fn is_valid(flags: u8) -> bool {
    !contains(
        flags,
        DIVISION_REPLACED_BY_RIGHT_SHIFT | MULTIPLICATION_REPLACED_BY_LEFT_SHIFT,
        ...
        && (!contains(flags, MULTIPLICATION_REPLACED_BY_LEFT_SHIFT)
            || contains(flags, PRE_MULTIPLICATION))
...
fn get_command(flags: u8, quiet: bool) -> String {
    ...
    if contains(
        flags,
        PRE_MULTIPLICATION | MULTIPLICATION_REPLACED_BY_LEFT_SHIFT,
    ) {
        index += 6
    }
...
fn test_primitive_compilation(flags: u8, quiet: bool) {
    ...
    match compile_code(command.as_str()) {
        Ok(actual_bytecode) => {
            ...
            assert_eq!(0, actual_bytecode.cell().reference_count());
            assert_eq!(&expected_bytecode, &actual_bytecode.cell().data());
        }
        Err(e) => {

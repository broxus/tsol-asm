...
use crate::{
    compile_code, compile_code_to_cell, complex::compile_slice, parse::parse_slice, CompileError,
    Engine, Position,
};
...
fn test_pushslice() {
    assert_eq!(
        compile_code("PUSHSLICE x5").unwrap().cell().data(),
        vec![0x8B, 0x15, 0x80]
    );
    assert_eq!(
        compile_code("PUSHSLICE x5_").unwrap().cell().data(),
        vec![0x8B, 0x05]
    );
    assert_eq!(
        compile_code("PUSHSLICE x8_").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x08_").unwrap().cell().data(),
        vec![0x8B, 0x10, 0x80]
    );
    assert_eq!(
        compile_code("PUSHSLICE x008_").unwrap().cell().data(),
        vec![0x8B, 0x10, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x0008_").unwrap().cell().data(),
        vec![0x8B, 0x20, 0x00, 0x80]
    );
    assert_eq!(
        compile_code("PUSHSLICE x00008_").unwrap().cell().data(),
        vec![0x8B, 0x20, 0x00, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x_").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x0_").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x00_").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x000_").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x0000_").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x00000_").unwrap().cell().data(),
        vec![0x8B, 0x08]
    );
    assert_eq!(
        compile_code("PUSHSLICE x0123456789012345678901234567890_")
            .unwrap()
            .cell()
            .data(),
        vec![
            0x8B, 0xF0, 0x12, 0x34, 0x56, 0x78, 0x90, 0x12, 0x34, 0x56, 0x78, 0x90, 0x12, 0x34,
            0x56, 0x78, 0x90
        ]
    );
}
...
fn test_incorrect_command() {
    assert_eq!(
        compile_code("AAAAAA").err().unwrap(),
...
fn test_comments_parsing_bug() {
    // https://github.com/everx-labs/ever-assembler/issues/28
    ...
    // 800f PUSHINT 15
    // 30   DROP
    assert_eq!(hex::encode(code.cell().data()), "77800f30");
}
...
mod test_pushslice {
    ...
    fn test_short_max() {
        ...
        let code = compile_code(&code[..]).unwrap();
        let bytecode = code.cell().data();
        assert_eq!(&bytecode[..4], &[0x8B, 0xF0, 0x00, 0x10]);
    }
    ...
    fn test_max() {
        ...
        let code = compile_code(&code[..]).unwrap();
        let bytecode = code.cell().data();
        assert_eq!(&bytecode[..4], &[0x8D, 0x1F, 0x00, 0x00]);
    }
}
...
mod test_stsliceconst {
    ...
    fn test_max() {
        ...
        let code = compile_code(&code[..]).unwrap();
        let bytecode = code.cell().data();
        assert_eq!(&bytecode[..3], &[0xCF, 0x9C, 0x00]);
    }
}
...
fn test_ifrefelseref() {
    ...
    .unwrap();
    assert_eq!(code.reference_count(), 2);
    assert_eq!(hex::encode(code.data()), "e30f");
    assert_eq!(code.reference(0).unwrap().reference_count(), 0);
    assert_eq!(hex::encode(code.reference(0).unwrap().data()), "f2c064");
    assert_eq!(code.reference(1).unwrap().reference_count(), 0);
    assert_eq!(hex::encode(code.reference(1).unwrap().data()), "f2c0c8");
}

#[test]
fn test_compile_dictpushconst() {
    let code = compile_code_to_cell(
        "
        ...
        .fragment test_create_at, {
            PUSHINT 2
        }
        DICTPUSHCONST 32, {
            x68b55f3f = constructor,
            x795e3bbc = test1a,
            x7aa36f61 = test_create_at,
        }
    ",
    )
    .unwrap();
    assert_eq!(code.reference_count(), 1);
    assert_eq!(hex::encode(code.data()), "f4a420");
}

mod test_dbginfo {
    use crate::debug::{DbgInfo, DbgNode, DbgPos};
    use tycho_types::prelude::CellBuilder;

    #[test]
    fn test_serdes() {
        let mut b = CellBuilder::new();
        b.store_raw(&[0x00, 0x01, 0x02, 0x03], 32).unwrap();
...
fn test_inline() {
    ...
    assert_eq!(
        hex::decode("927172").unwrap(),
        unit.finalize().0.cell().data()
    );
}

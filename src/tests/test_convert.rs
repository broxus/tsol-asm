...
fn from_big_endian_octet_stream(mut load_u8: impl FnMut() -> Result<u8>) -> Result<BigInt> {
    let first_byte = load_u8()?;
    ...
    for _ in 1..upper {
        value <<= 8;
        value |= load_u8()? as u32;
    }
    let last_index = digit_count - 1;
    digits[last_index] = value;

    for i in (0..last_index).rev() {
        let mut value = (load_u8()? as u32) << 24;
        value |= (load_u8()? as u32) << 16;
        value |= (load_u8()? as u32) << 8;
        value |= load_u8()? as u32;
...
fn decimal_from_str_big_negative() {
    test(
        ...
        &[
            0x6F, 0xE8, 0x39, 0x1C, 0x3F, 0xCD, 0x07, 0x6F, 0xBA, 0x52, 0x8B, 0x99, 0x7B, 0xFB,
            0xA0, 0xEB,
        ],
    );
    test(
        ...
        &[
            0xA6, 0xB6, 0x07, 0x6F, 0xDF, 0xB4, 0xDB, 0x4A, 0x86, 0x86, 0xd7, 0xB2, 0xBF, 0x9F,
            0x1E, 0xF8, 0x17, 0x8F, 0x49, 0x7B, 0xFB, 0xA0, 0xEB,
        ],
    );
}
...
fn decimal_from_str_big_positive() {
    test(
        "123456789123456789123456789123456789",
        &[
            0x68, 0x17, 0xC6, 0xE3, 0xC0, 0x32, 0xF8, 0x90, 0x45, 0xAD, 0x74, 0x66, 0x84, 0x04,
            0x5F, 0x15,
        ],
    );
    test(
        "123456789123456789123456789123456789123456789123456789",
        &[
            0xA1, 0x49, 0xF8, 0x90, 0x20, 0x4B, 0x24, 0xB5, 0x79, 0x79, 0x28, 0x4D, 0x40, 0x60,
            0xE1, 0x07, 0xE8, 0x70, 0xB6, 0x84, 0x04, 0x5F, 0x15,
        ],
    );
}

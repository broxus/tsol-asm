...
use super::types::{Code, Instruction, InstructionParameter, OperationBehavior};
use super::Result;
use crate::fail;
use crate::slice_to_owned_slice;
use crate::trim_zeroes_right;
use num_traits::Zero;
...
macro_rules! create_handler_1 {
    ($func_name:ident, $opc:literal, $mnemonic:literal) => {
        pub(super) fn $func_name(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
            let opc = slice.load_uint(8)?;
            if opc != $opc {
                fail!("invalid opcode");
            }
...
macro_rules! create_handler_1t {
    ($func_name:ident, $opc:literal, $mnemonic:literal) => {
        pub(super) fn $func_name<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
        where
            T: OperationBehavior,
        {
            let opc = slice.load_uint(8)?;
...
macro_rules! create_handler_2r {
    ($func_name:ident, $opc:literal, $mnemonic:literal) => {
        pub(super) fn $func_name(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
            ...
            let cell = slice.load_reference_cloned().ok();
...
macro_rules! create_handler_3r {
    ($func_name:ident, $opc:literal, $mnemonic:literal) => {
        pub(super) fn $func_name(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
            ...
            let cell1 = slice.load_reference_cloned().ok();
            let cell2 = slice.load_reference_cloned().ok();
            let code1 = self.load_cell(&cell1)?;
            let code2 = self.load_cell(&cell2)?;
            Ok(Instruction::new($mnemonic)
                .with_refs(2)
                .with_param(InstructionParameter::Code {
                    code: code1,
                    cell: cell1,
                })
                .with_param(InstructionParameter::Code {
...
impl Loader {
    ...
    pub fn load(&mut self, slice: &mut CellSlice, inline: bool) -> Result<Code> {
        let orig_slice = slice.clone();
        let mut code = match self.load_slice(slice) {
            Ok(code) => code,
            Err(_) => {
                // failed to load the slice - emit it as-is
                let mut insns = Code::single(Instruction::new(".blob").with_param(
                    InstructionParameter::Slice(slice_to_owned_slice(orig_slice)),
                ));
                for i in 0..orig_slice.size_refs() {
                    insns.push(
                        Instruction::new(".cell").with_param(InstructionParameter::Cell {
                            cell: orig_slice.get_reference_cloned(i).ok(),
                            collapsed: false,
                        }),
                    )
                }
        ...
        match slice.size_refs().cmp(&1) {
            Ordering::Less => (),
            Ordering::Equal => {
                let next_cell = slice.load_reference_cloned().ok();
    ...
    fn load_slice(&mut self, slice: &mut CellSlice) -> Result<Code> {
        ...
        while slice.size_bits() > 0 {
            ...
            let bits = bytecode.size_bits() - slice.size_bits();
            let refs = bytecode.size_refs() - slice.size_refs();
            bytecode.only_first(bits, refs)?;
            insn.set_bytecode(slice_to_owned_slice(bytecode));
    ...
    fn load_cell_impl(&mut self, cell: &Cell) -> Result<Code> {
        if let Some(code) = self.history.get(&cell.repr_hash().0) {
            if self.collapse {
                Ok(Code::single(Instruction::new(";;").with_param(
            ...
            let code = self.load(&mut slice, false).unwrap_or_else(|_| {
            ...
            self.history.insert(cell.repr_hash().0, code.clone());
            Ok(code)
        }
    }
    pub(super) fn unknown(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        fail!("unknown opcode {}", slice.display_data().to_string())
    }
    pub(super) fn setcp(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xff);
        let cp = slice.load_u8()?;
    ...
    pub(super) fn xchg_simple(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(4)?;
        check!(opc == 0 || opc == 1);
        let i = slice.load_uint(4)? as isize;
        match opc {
    ...
    pub(super) fn xchg_std(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x10);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
        Ok(Instruction::new("XCHG").with_param(InstructionParameter::StackRegisterPair(i, j)))
    }
    pub(super) fn xchg_long(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x11);
        let ii = slice.load_uint(8)? as isize;
        Ok(Instruction::new("XCHG").with_param(InstructionParameter::StackRegisterPair(0, ii)))
    }
    pub(super) fn push_simple(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(4)?;
        check_eq!(opc, 0x2);
        let i = slice.load_uint(4)? as isize;
        Ok(Instruction::new("PUSH").with_param(InstructionParameter::StackRegister(i)))
    }
    pub(super) fn pop_simple(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(4)?;
        check_eq!(opc, 0x3);
        let i = slice.load_uint(4)? as isize;
        Ok(Instruction::new("POP").with_param(InstructionParameter::StackRegister(i)))
    }
    pub(super) fn xchg3(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(4)?;
        check_eq!(opc, 0x4);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
        let k = slice.load_uint(4)? as isize;
        Ok(
            Instruction::new("XCHG3")
                .with_param(InstructionParameter::StackRegisterTriple(i, j, k)),
        )
    }
    pub(super) fn xchg2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x50);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
    ...
    pub(super) fn xcpu(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x51);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
    ...
    pub(super) fn puxc(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x52);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
    ...
    pub(super) fn push2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x53);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
    ...
    pub(super) fn xc2pu(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x541);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
        let k = slice.load_uint(4)? as isize;
    ...
    pub(super) fn xcpuxc(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x542);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
        let k = slice.load_uint(4)? as isize;
        Ok(
            Instruction::new("XCPUXC").with_param(InstructionParameter::StackRegisterTriple(
                i,
                j,
                k - 1,
            )),
    ...
    pub(super) fn xcpu2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x543);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
    ...
    pub(super) fn puxc2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x544);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
        let k = slice.load_uint(4)? as isize;
        Ok(
            Instruction::new("PUXC2").with_param(InstructionParameter::StackRegisterTriple(
                i,
                j - 1,
                k - 1,
            )),
    ...
    pub(super) fn puxcpu(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x545);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
        let k = slice.load_uint(4)? as isize;
        Ok(
            Instruction::new("PUXCPU").with_param(InstructionParameter::StackRegisterTriple(
                i,
                j - 1,
                k - 1,
            )),
    ...
    pub(super) fn pu2xc(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x546);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
        let k = slice.load_uint(4)? as isize;
        Ok(
            Instruction::new("PU2XC").with_param(InstructionParameter::StackRegisterTriple(
                i,
                j - 1,
                k - 2,
            )),
    ...
    pub(super) fn push3(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x547);
        let i = slice.load_uint(4)? as isize;
        let j = slice.load_uint(4)? as isize;
    ...
    pub(super) fn blkswap(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x55);
        let i = slice.load_uint(4)? as usize;
        let j = slice.load_uint(4)? as usize;
        Ok(Instruction::new("BLKSWAP")
            .with_param(InstructionParameter::LengthAndIndex(i + 1, j + 1)))
    }
    pub(super) fn push(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x56);
        let ii = slice.load_uint(8)? as isize;
        Ok(Instruction::new("PUSH").with_param(InstructionParameter::StackRegister(ii)))
    }
    pub(super) fn pop(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x57);
        let ii = slice.load_uint(8)? as isize;
    ...
    pub(super) fn reverse(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x5e);
        let i = slice.load_uint(4)? as usize;
        let j = slice.load_uint(4)? as usize;
    ...
    pub(super) fn blkdrop(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x5f0);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("BLKDROP").with_param(InstructionParameter::Length(i)))
    }
    pub(super) fn blkpush(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x5f);
        let i = slice.load_uint(4)? as usize;
        let j = slice.load_uint(4)? as usize;
    ...
    pub(super) fn blkdrop2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x6c);
        let i = slice.load_uint(4)? as usize;
        check!(i > 0);
    ...
    pub(super) fn tuple_create(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f0);
        let k = slice.load_uint(4)? as usize;
        Ok(Instruction::new("TUPLE").with_param(InstructionParameter::Length(k)))
    }
    pub(super) fn tuple_index(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f1);
        let k = slice.load_uint(4)? as usize;
        Ok(Instruction::new("INDEX").with_param(InstructionParameter::Length(k)))
    }
    pub(super) fn tuple_un(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f2);
        let k = slice.load_uint(4)? as usize;
        Ok(Instruction::new("UNTUPLE").with_param(InstructionParameter::Length(k)))
    }
    pub(super) fn tuple_unpackfirst(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f3);
        let k = slice.load_uint(4)? as usize;
        Ok(Instruction::new("UNPACKFIRST").with_param(InstructionParameter::Length(k)))
    }
    pub(super) fn tuple_explode(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f4);
        let n = slice.load_uint(4)? as usize;
        Ok(Instruction::new("EXPLODE").with_param(InstructionParameter::Length(n)))
    }
    pub(super) fn tuple_setindex(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f5);
        let k = slice.load_uint(4)? as usize;
        Ok(Instruction::new("SETINDEX").with_param(InstructionParameter::Length(k)))
    }
    pub(super) fn tuple_index_quiet(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f6);
        let k = slice.load_uint(4)? as usize;
        Ok(Instruction::new("INDEXQ").with_param(InstructionParameter::Length(k)))
    }
    pub(super) fn tuple_setindex_quiet(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6f7);
        let k = slice.load_uint(4)? as usize;
    ...
    pub(super) fn tuple_index2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0x6fb);
        let i = slice.load_uint(2)? as isize;
        let j = slice.load_uint(2)? as isize;
        Ok(Instruction::new("INDEX2").with_param(InstructionParameter::StackRegisterPair(i, j)))
    }
    pub(super) fn tuple_index3(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(10)?;
        check_eq!(opc << 2, 0x6fe);
        let i = slice.load_uint(2)? as isize;
        let j = slice.load_uint(2)? as isize;
        let k = slice.load_uint(2)? as isize;
        Ok(Instruction::new("INDEX3")
            .with_param(InstructionParameter::StackRegisterTriple(i, j, k)))
    }
    pub(super) fn pushint(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        ...
        if opc <= 0x7a {
            ...
            x = (slice.load_uint(8)? as i8) as i16;
        } else if opc == 0x81 {
            x = slice.load_uint(16)? as i16;
        }
    ...
    fn bigint(&mut self, slice: &mut CellSlice) -> Result<num::BigInt> {
        ...
        for _ in 1..upper {
            value <<= 8;
            value |= slice.load_u8()? as u32;
        }
        let last_index = digit_count - 1;
        digits[last_index] = value;

        for i in (0..last_index).rev() {
            let mut value = (slice.load_u8()? as u32) << 24;
            value |= (slice.load_u8()? as u32) << 16;
            value |= (slice.load_u8()? as u32) << 8;
            value |= slice.load_u8()? as u32;
    ...
    pub(super) fn pushint_big(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x82);
        let int = self.bigint(slice)?;
        Ok(Instruction::new("PUSHINT").with_param(InstructionParameter::BigInteger(int)))
    }
    pub(super) fn pushpow2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x83);
        let xx = slice.load_uint(8)? as isize;
        Ok(Instruction::new("PUSHPOW2").with_param(InstructionParameter::Integer(xx + 1)))
    }
    create_handler_2!(pushnan, 0x83ff, "PUSHNAN");
    pub(super) fn pushpow2dec(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x84);
        let xx = slice.load_uint(8)? as isize;
        Ok(Instruction::new("PUSHPOW2DEC").with_param(InstructionParameter::Integer(xx + 1)))
    }
    pub(super) fn pushnegpow2(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x85);
        let xx = slice.load_uint(8)? as isize;
        Ok(Instruction::new("PUSHNEGPOW2").with_param(InstructionParameter::Integer(xx + 1)))
    }
    pub(super) fn pushref(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x88);
        let cell = slice.load_reference_cloned().ok();
        Ok(Instruction::new("PUSHREF")
            .with_refs(1)
            .with_param(InstructionParameter::Cell {
                cell,
                collapsed: false,
            }))
    }
    pub(super) fn pushrefslice(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x89);
        let cell = slice.load_reference_cloned().ok();
    ...
    pub(super) fn pushrefcont(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x8a);
        let cell = slice.load_reference_cloned().ok();
        let code = self.load_cell(&cell)?;
    ...
    pub(super) fn pushslice_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x8b);
        let x = slice.load_uint(4)? as u16;
        let mut bitstring = slice.load_prefix(x * 8 + 4, 0)?;
        trim_zeroes_right(&mut &mut bitstring);
        Ok(Instruction::new("PUSHSLICE")
            .with_param(InstructionParameter::Slice(slice_to_owned_slice(bitstring))))
    }
    pub(super) fn pushslice_mid(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x8c);
        let r = slice.load_uint(2)? as usize;
        let xx = slice.load_uint(5)? as usize;
        let bits = xx * 8 + 1;

        let mut subslice = slice.clone();
        subslice
            .only_first(
                std::cmp::min(bits as u16, subslice.size_bits()),
                std::cmp::min(r as u8, subslice.size_refs()),
            )
            .unwrap();
        let missing_bits = bits - subslice.size_bits() as usize;
        let missing_refs = r - subslice.size_refs() as usize;
        trim_zeroes_right(&mut subslice);

        slice
            .only_last(
                slice.size_bits() + missing_bits as u16 - bits as u16,
                slice.size_refs() + missing_refs as u8 - r as u8,
            )
            .unwrap();

        let mut insn = Instruction::new("PUSHSLICE")
            .with_refs(r)
            .with_param(InstructionParameter::Slice(slice_to_owned_slice(subslice)));
        comment_missing_bits_and_refs(&mut insn, missing_bits, missing_refs);
        Ok(insn)
    }
    pub(super) fn pushslice_long(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0x8d);
        let r = slice.load_uint(3)? as usize;
        let xx = slice.load_uint(7)? as usize;
        let bits = xx * 8 + 6;
    ...
    pub(super) fn pushcont_long(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(7)?;
        check_eq!(opc << 1, 0x8e);
        let r = slice.load_uint(2)? as usize;
        let xx = slice.load_uint(7)? as usize;
        let bits = xx * 8;
    ...
    pub(super) fn pushcont_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(4)?;
        check_eq!(opc, 0x9);
        let x = slice.load_uint(4)? as usize;
        let mut body = slice.load_prefix((x * 8) as u16, 0)?;
        let code = self.load(&mut body, true)?;
        Ok(
            Instruction::new("PUSHCONT")
                .with_param(InstructionParameter::Code { code, cell: None }),
        )
    }
    ...
    pub(super) fn addconst<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xa6);
        let cc = slice.load_uint(8)? as i8;
        Ok(T::insn(Instruction::new("ADDCONST"))
            .with_param(InstructionParameter::Integer(cc as isize)))
    }
    pub(super) fn mulconst<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
    ...
    pub(super) fn divmod<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xa9);
        let opc2 = slice.load_uint(8)?;
        match opc2 {
            ...
            0x34 => {
                let tt = slice.load_uint(8)? as usize;
    ...
    pub(super) fn lshift<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
        match opc {
            0xaa => {
                let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn rshift<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
    ...
    pub(super) fn fits<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
    ...
    pub(super) fn ufits<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
    ...
    pub(super) fn eqint<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xc0);
        let yy = (slice.load_uint(8)? as i8) as isize;
        Ok(T::insn(Instruction::new("EQINT")).with_param(InstructionParameter::Integer(yy)))
    }
    pub(super) fn lessint<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
    ...
    pub(super) fn gtint<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
    ...
    pub(super) fn neqint<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(8)?;
    ...
    pub(super) fn sti(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xca);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("STI").with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn stu(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xcb);
        let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn stir(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf0a);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("STIR").with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn stur(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf0b);
        let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn stiq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf0c);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("STI")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn stuq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf0d);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("STU")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn stirq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf0e);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("STIR")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn sturq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf0f);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("STUR")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    ...
    pub(super) fn bchkbits_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf38);
        let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn bchkbitsq_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xcf3c);
        let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn stsliceconst(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(9)?;
        check_eq!(opc << 3, 0xcf8);
        let x = slice.load_uint(2)?;
        check_eq!(x, 0);
        let y = slice.load_uint(3)?;
        let mut sss = slice.load_prefix(y as u16 * 8 + 2, 0)?;
        trim_zeroes_right(&mut sss);
        Ok(Instruction::new("STSLICECONST")
            .with_param(InstructionParameter::Slice(slice_to_owned_slice(sss))))
    }
    create_handler_1!(ctos, 0xd0, "CTOS");
    create_handler_1!(ends, 0xd1, "ENDS");
    pub(super) fn ldi(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xd2);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("LDI").with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn ldu(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xd3);
        let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn ldslice(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xd6);
        let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn pldi(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd70a);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("PLDI").with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn pldu(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd70b);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("PLDU").with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn ldiq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd70c);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("LDI")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn lduq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd70d);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("LDU")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn pldiq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd70e);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("PLDI")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn plduq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd70f);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("PLDU")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn plduz(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(13)?;
        check_eq!(opc << 3, 0xd710);
        let c = slice.load_uint(3)? as usize;
    ...
    pub(super) fn pldslice(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd71d);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("PLDSLICE").with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn ldsliceq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd71e);
        let cc = slice.load_uint(8)? as usize;
        Ok(Instruction::new("LDSLICE")
            .set_quiet()
            .with_param(InstructionParameter::Length(cc + 1)))
    }
    pub(super) fn pldsliceq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xd71f);
        let cc = slice.load_uint(8)? as usize;
    ...
    pub(super) fn sdbegins(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(14)?;
        check_eq!(opc << 2, 0xd728);
        let x = slice.load_uint(7)? as usize;
        let mut bitstring = slice.load_prefix(8 * x as u16 + 3, 0)?;
        trim_zeroes_right(&mut bitstring);
        Ok(Instruction::new("SDBEGINS")
            .with_param(InstructionParameter::Slice(slice_to_owned_slice(bitstring))))
    }
    pub(super) fn sdbeginsq(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(14)?;
    ...
    pub(super) fn pldrefidx(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(14)?;
        check_eq!(opc << 2, 0xd74c);
        let n = slice.load_uint(2)? as usize;
    ...
    pub(super) fn callxargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        match opc {
            0xda => {
                let p = slice.load_uint(4)? as usize;
                let r = slice.load_uint(4)? as usize;
                Ok(Instruction::new("CALLXARGS")
                    .with_param(InstructionParameter::Pargs(p))
                    .with_param(InstructionParameter::Rargs(r)))
            }
            0xdb => {
                let z = slice.load_uint(4)?;
                check_eq!(z, 0);
                let p = slice.load_uint(4)? as usize;
                Ok(Instruction::new("CALLXARGS")
                    .with_param(InstructionParameter::Pargs(p))
    ...
    pub(super) fn jmpxargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xdb1);
        let p = slice.load_uint(4)? as usize;
        Ok(Instruction::new("JMPXARGS").with_param(InstructionParameter::Pargs(p)))
    }
    pub(super) fn retargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xdb2);
        let r = slice.load_uint(4)? as usize;
    ...
    pub(super) fn callccargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(16)?;
        check_eq!(opc, 0xdb36);
        let p = slice.load_uint(4)? as usize;
        let r = slice.load_uint(4)? as usize;
    ...
    pub(super) fn ifbitjmp(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(15)?;
        check_eq!(opc << 1, 0xe38);
        let n = slice.load_uint(5)? as isize;
        Ok(Instruction::new("IFBITJMP").with_param(InstructionParameter::Integer(n)))
    }
    pub(super) fn ifnbitjmp(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(15)?;
        check_eq!(opc << 1, 0xe3a);
        let n = slice.load_uint(5)? as isize;
        Ok(Instruction::new("IFNBITJMP").with_param(InstructionParameter::Integer(n)))
    }
    pub(super) fn ifbitjmpref(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(15)?;
    ...
    pub(super) fn ifnbitjmpref(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(15)?;
    ...
    pub(super) fn setcontargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xec);
        let r = slice.load_uint(4)? as usize;
        let n = slice.load_uint(4)? as isize;
        Ok(Instruction::new("SETCONTARGS")
            .with_param(InstructionParameter::Rargs(r))
            .with_param(InstructionParameter::Nargs(n)))
    }
    pub(super) fn returnargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xed0);
        let p = slice.load_uint(4)? as usize;
    ...
    pub(super) fn pushctr(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xed4);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("PUSHCTR").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn popctr(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xed5);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("POPCTR").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn setcontctr(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xed6);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("SETCONTCTR").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn setretctr(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xed7);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("SETRETCTR").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn setaltctr(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xed8);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("SETALTCTR").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn popsave(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xed9);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("POPSAVE").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn save(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xeda);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("SAVE").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn savealt(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xedb);
        let i = slice.load_uint(4)? as usize;
        Ok(Instruction::new("SAVEALT").with_param(InstructionParameter::ControlRegister(i)))
    }
    pub(super) fn saveboth(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xedc);
        let i = slice.load_uint(4)? as usize;
    ...
    pub(super) fn blessargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xee);
        let r = slice.load_uint(4)? as usize;
        let n = slice.load_uint(4)? as isize;
    ...
    pub(super) fn call_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xf0);
        let n = slice.load_uint(8)? as isize;
        Ok(Instruction::new("CALL").with_param(InstructionParameter::Nargs(n)))
    }
    pub(super) fn call_long(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(10)?;
        check_eq!(opc << 2, 0xf10);
        let n = slice.load_uint(14)? as isize;
        Ok(Instruction::new("CALL").with_param(InstructionParameter::Nargs(n)))
    }
    pub(super) fn jmp(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(10)?;
        check_eq!(opc << 2, 0xf14);
        let n = slice.load_uint(14)? as isize;
        Ok(Instruction::new("JMPDICT").with_param(InstructionParameter::Nargs(n)))
    }
    pub(super) fn prepare(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(10)?;
        check_eq!(opc << 2, 0xf18);
        let n = slice.load_uint(14)? as isize;
        Ok(Instruction::new("PREPARE").with_param(InstructionParameter::Nargs(n)))
    }
    pub(super) fn throw_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(10)?;
        check_eq!(opc << 2, 0xf20);
        let nn = slice.load_uint(6)? as isize;
        Ok(Instruction::new("THROW").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throwif_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(10)?;
        check_eq!(opc << 2, 0xf24);
        let nn = slice.load_uint(6)? as isize;
        Ok(Instruction::new("THROWIF").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throwifnot_short(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(10)?;
        check_eq!(opc << 2, 0xf28);
        let nn = slice.load_uint(6)? as isize;
        Ok(Instruction::new("THROWIFNOT").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throw_long(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(13)?;
        check_eq!(opc << 3, 0xf2c0);
        let nn = slice.load_uint(11)? as isize;
        Ok(Instruction::new("THROW").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throwarg(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(13)?;
        check_eq!(opc << 3, 0xf2c8);
        let nn = slice.load_uint(11)? as isize;
        Ok(Instruction::new("THROWARG").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throwif_long(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(13)?;
        check_eq!(opc << 3, 0xf2d0);
        let nn = slice.load_uint(11)? as isize;
        Ok(Instruction::new("THROWIF").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throwargif(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(13)?;
        check_eq!(opc << 3, 0xf2d8);
        let nn = slice.load_uint(11)? as isize;
        Ok(Instruction::new("THROWARGIF").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throwifnot_long(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(13)?;
        check_eq!(opc << 3, 0xf2e0);
        let nn = slice.load_uint(11)? as isize;
        Ok(Instruction::new("THROWIFNOT").with_param(InstructionParameter::Integer(nn)))
    }
    pub(super) fn throwargifnot(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(13)?;
    ...
    pub(super) fn tryargs(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(8)?;
        check_eq!(opc, 0xf3);
        let p = slice.load_uint(4)? as usize;
        let r = slice.load_uint(4)? as usize;
    ...
    pub(super) fn ldmsgaddr<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(16)?;
    ...
    pub(super) fn parsemsgaddr<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(16)?;
    ...
    pub(super) fn rewrite_std_addr<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(16)?;
    ...
    pub(super) fn rewrite_var_addr<T>(&mut self, slice: &mut CellSlice) -> Result<Instruction>
    where
        T: OperationBehavior,
    {
        let opc = slice.load_uint(16)?;
    ...
    pub(super) fn dictpushconst(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(14)?;
        check_eq!(opc << 2, 0xf4a4);
        let n = slice.load_uint(10)? as usize;
        let cell = slice.load_reference_cloned().ok();
    ...
    pub(super) fn pfxdictswitch(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(14)?;
    ...
    pub(super) fn getparam(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xf82);
        let i = slice.load_uint(4)? as usize;
    ...
    pub(super) fn getglob(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(11)?;
        check_eq!(opc << 1, 0xf84);
        let k = slice.load_uint(5)? as usize;
        assert_ne!(k, 0);
        Ok(Instruction::new("GETGLOB").with_param(InstructionParameter::Length(k)))
    }
    create_handler_2!(setglobvar, 0xf860, "SETGLOBVAR");
    pub(super) fn setglob(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(11)?;
    ...
    pub(super) fn dump_stack_top(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xfe0);
        let n = slice.load_uint(4)? as isize;
        assert!(n > 0);
    ...
    pub(super) fn dump_var(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xfe2);
        let n = slice.load_uint(4)? as isize;
        assert!(n < 15);
    ...
    pub(super) fn print_var(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xfe3);
        let n = slice.load_uint(4)? as isize;
        assert!(n < 15);
    ...
    pub(super) fn dump_string(&mut self, slice: &mut CellSlice) -> Result<Instruction> {
        let opc = slice.load_uint(12)?;
        check_eq!(opc, 0xfef);
        let n = slice.load_uint(4)?;
        let mode = slice.load_uint(8)?;
        match n {
            0 => {
                check_eq!(mode, 0x00);
            ...
            _ => {
                if mode == 0x00 {
                    let s = slice.load_prefix(n as u16 * 8, 0)?;
                    Ok(Instruction::new("LOGSTR")
                        .with_param(InstructionParameter::Slice(slice_to_owned_slice(s))))
                } else if mode == 0x01 {
                    let s = slice.load_prefix(n as u16 * 8, 0)?;
                    Ok(Instruction::new("PRINTSTR")
                        .with_param(InstructionParameter::Slice(slice_to_owned_slice(s))))
                } else {
